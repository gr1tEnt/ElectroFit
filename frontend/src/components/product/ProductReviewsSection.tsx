      ...
      {!loading && !loadError && (
        <div className="mt-6 grid gap-8 lg:grid-cols-[minmax(280px,340px)_1fr] lg:items-start">
          ...
          <div>
            {reviews.length === 0 ? (
              <EmptyState
                title="Ще немає відгуків. Станьте першим, хто оцінить цей товар!"
                description="Поділіться враженнями після покупки — це допоможе іншим обрати якісний товар."
                icon="⭐"
              />
            ) : (
        ...
        {isAuthenticated && (
          <div className="rounded-2xl border border-border bg-slate-50 p-4 sm:p-6">
            ...
            <form onSubmit={handleSubmit} className="mt-4 space-y-4">
              ...
              >
                {submitting ? "Обробка…" : "Надіслати відгук"}
              </button>
            </form>
          </div>
        )}
